<?
	...
	if (!empty($_GET["id"]))
	{
		$commerc = get_bild($_GET["id"]);
				
		$smarty->assign('commerc', $commerc);

		$oblast = $commerc["adr_obl"];
		
		$mista = get_table_info("d_mista", "parent=".$commerc["adr_rgn"]);
  ...
  if (mysql_num_rows($res)>0){
	...
	$smarty->assign('page_name', "commerc_edit");
