...
<div style="border:1px dotted green;height:20px;margin:4px;padding:4px" id='sdiv'><div onclick="ShowHideDiv('findforms');" style="cursor:pointer;float:left" width=50%><button>Відбір &#8595;</button></div><div style="float:right;"><button  onclick="window.location='apartment_edit.php?panel=kvaadd';">Додати</button></div></div>
<?php
...
<tr style="background-color: #BDCACC"><th rowspan=2>№</th><th>Місто</th><th>Кімнати</th><th>Поверх</th><th>Ціна</th><th>Правка</th></tr>
<tr style="background-color: #BDCACC"><th>Вулиця</th><th>Площі</th><th>Огол.</th><th>Агент</th><th>Знищити</th></tr>
<?php
	$a=1;
//if ($page!='all')
...
//else
  $sql="Select * from m_bildings $usl order by id;";
    ...
    $params = '&nomer='.$nomer.'&agent='.$agent.'&obl='.$obl.'&rgn='.$rgn.'&mista='.$mista;
		echo '<td><a href="apartment_edit.php?id='.$row['id'].$params.'"><img class=aimg src="/i/edit.png" style="cursor:pointer;border:0"></td>';
    ...
    echo '<td>'.$row['adr_vul'].'</td>';
		...
		echo '<td><a href="obj_del.php?id='.$row['id'].'" onclick="return confirm(\'Знищити квартиру?\')"><img class=aimg src="/i/delete.png"></a></td>';
