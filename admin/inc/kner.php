...
?>
<div style="border:1px dotted green;height:20px;margin:4px;padding:4px" id='sdiv'><div onclick="ShowHideDiv('findforms');" style="cursor:pointer;float:left" width=50%><button>Відбір &#8595;</button></div><div style="float:right;"><button onclick="window.location='commerc_edit.php?panel=kneradd';">Додати</button></div></div>
<?php
...
<tr bgcolor=#BDCACC><th rowspan=2>№</th><th>Адреса</th><th>Кімнати</th><th>Поверх</th><th>Ціна</th><th>Правка</th></tr>
<tr bgcolor=#BDCACC><th>Площі</th><th>Площі</th><th>Оголош.</th><th>Агент</th><th>Знищити</th></tr>
<?php
	$a=1;
  ...
  if ($page=='all')
    ...
    $params = '&nomer='.$nomer.'&agent='.$agent.'&obl='.$obl.'&rgn='.$rgn.'&mista='.$mista;
		echo '<td><a href="commerc_edit.php?id='.$row['id'].$params.'"><img class=aimg src="./i/edit.gif" style="cursor:pointer;border:0"></td>';
    ...
    echo '<td>'.$row['adr_vul'].'</td>';
		...
		echo '<td><a href="obj_del.php?id='.$row['id'].'" onclick="return confirm(\'Знищити об`єкт?\')"><img class=aimg src="./i/del.gif"></a></td>';
