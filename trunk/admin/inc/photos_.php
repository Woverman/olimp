...
if (isset($_POST['file_to_del'])){ //удаление фоток
  $fname=$config['SIGHT_ROOT'].'/i/site/'.$_POST['file_to_del'];
  @unlink ($fname);
  $fname=$config['SIGHT_ROOT'].'/i/site-p/'.$_POST['file_to_del'];
  @unlink ($fname);
}

if (isset($_POST['mode'])){
  $tmpfile=$_FILES['foto']['tmp_name'];
  $newname=$config['SIGHT_ROOT'].'/i/site/'.$_FILES['foto']['name'];
  $thumbnail=$config['SIGHT_ROOT'].'/i/site-p/'.$_FILES['foto']['name'];
...
$dir=$config['SIGHT_ROOT'].'/i/site-p';
$files=showDir($dir);
if (is_array($files)){
foreach ($files as $file){
  list($width, $height, $type, $attr) = getimagesize($config['SIGHT_ROOT'].'/i/site/'.$file);
  if ($width>$height) {
    $w=120;
    $h=86;
    }
  ...
  <img class=award_smoll width=<?=$w?> height=<?=$h?> src="/i/site-p/<?=$file?>" style="margin-bottom:<?=122-$h?>">
  <br>
